...
module OpenapiClient
  class VerificationGovernmentIdAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::VerificationGovernmentIdAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::VerificationGovernmentIdAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [status, created_at, created_at_ts, submitted_at, submitted_at_ts, completed_at, completed_at_ts, country_code, checks, address_street_1, address_street_2, address_city, address_subdivision, address_postal_code, entity_confidence_reasons, entity_confidence_score, front_photo_url, back_photo_url, photo_urls, selfie_photo, selfie_photo_url, video_url, id_class, capture_method, name_first, name_middle, name_last, name_suffix, birthdate, issuing_authority, issuing_subdivision, nationality, document_number, visa_status, issue_date, expiration_date, designations, birthplace, endorsements, height, sex, restrictions, vehicle_class, identification_number].hash
    end
