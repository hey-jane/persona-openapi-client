...
module PersonaAPIClient
  class ApiKeyWithValueAttributes
    attr_accessor :name
    ...
    # The API Key value. This should be treated with the utmost security.
    attr_accessor :value
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PersonaAPIClient::ApiKeyWithValueAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PersonaAPIClient::ApiKeyWithValueAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, note, api_version, api_key_inflection, api_attributes_blocklist, permissions, ip_address_allowlist, file_access_token_expires_in, last_used_at, expires_at, created_at, value].hash
    end
