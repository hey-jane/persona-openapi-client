...
module PersonaAPIClient
  class ReportWatchlistAttributes
    ...
    # Lists that matched from the search input
    ...
    def self.openapi_types
      {
        ...
        :'birthdate' => :'String',
        :'country_code' => :'String',
        :'matched_lists' => :'Array<ReportWatchlistAttributesAllOfMatchedListsInner>',
        :'run_history' => :'Array<ReportRunHistoryInner>',
        :'ignore_list' => :'Array<ReportWatchlistAttributesAllOfIgnoreListInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PersonaAPIClient::ReportWatchlistAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PersonaAPIClient::ReportWatchlistAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ignore_list')
        if (value = attributes[:'ignore_list']).is_a?(Array)
    ...
    def hash
      [status, created_at, completed_at, redacted_at, report_template_version_name, has_match, is_continuous, is_recurring, tags, name_first, name_middle, name_last, term, birthdate, country_code, matched_lists, run_history, ignore_list].hash
    end
