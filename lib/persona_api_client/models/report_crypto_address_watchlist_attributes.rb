...
module PersonaAPIClient
  class ReportCryptoAddressWatchlistAttributes
    ...
    def self.openapi_types
      {
        ...
        :'tags' => :'Array<String>',
        :'query' => :'ReportCryptoAddressWatchlistAttributesAllOfQuery',
        :'result' => :'Array<ReportCryptoAddressWatchlistAttributesAllOfResultInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PersonaAPIClient::ReportCryptoAddressWatchlistAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PersonaAPIClient::ReportCryptoAddressWatchlistAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [status, created_at, completed_at, redacted_at, report_template_version_name, has_match, is_continuous, is_recurring, tags, query, result].hash
    end
